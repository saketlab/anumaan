...
Stan model (via cmdstanr) with correlated random effects at up to three
grouping levels (e.g. hospital, patient, admission). Two residual
structures are supported: identity (default, no residual correlation)
and correlated (full LKJ-Cholesky residual covariance). Run once per
